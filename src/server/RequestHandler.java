...
public class RequestHandler implements HttpHandler {
    ...
    private String handlePut(HttpExchange httpExchange) throws IOException {
        ...
        String language = putData.substring(
            0,
            putData.indexOf(",")
        ), year = putData.substring(putData.indexOf(",") + 1);

        String response;
        if(data.containsKey(language)){
            String yearp = data.get(language);
           response = "Updated entry {" + language + ", " + year + "} (previous year: " + yearp + ")";
        }
        else{
